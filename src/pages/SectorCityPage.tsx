...
const SectorCityPage = () => {
  ...
  // Extract the parent slug from the URL path (e.g. "/mas-clientes-para-fontaneros/madrid" → "mas-clientes-para-fontaneros")
  ...
  return (
    <>
        ...
        <div className="container grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <h1 className="font-heading text-3xl md:text-4xl leading-tight mb-4">
              Más {ctaWordCap} para tu {sector.relatedLabel.slice(0, -1) === sector.relatedLabel ? sector.relatedLabel : `Negocio de ${sector.relatedLabel}`} en {city.name}
            </h1>
      ...
      <section className="py-16">
        <div className="container max-w-3xl">
          <h2 className="font-heading text-2xl md:text-3xl mb-6">
            ¿Por qué los {sectorLabel} en {city.name} necesitan visibilidad en Google?
          </h2>
          <p className="text-muted-foreground leading-relaxed mb-4">{ctxData.contextLine1}</p>
          <p className="text-muted-foreground leading-relaxed">
            {ctxData.contextLine2} Para un negocio de {sectorLabel}, esto significa que cada búsqueda sin respuesta es un {sector.ctaWord === "alumnos" ? "alumno" : sector.ctaWord === "pacientes" ? "paciente" : "cliente"} que se va a la competencia.
          </p>
        </div>
      </section>
      ...
      <section className="bg-warm-bg py-16">
        <div className="container">
          <h2 className="font-heading text-2xl md:text-3xl mb-8">
            Qué activamos para {sectorLabel} en {city.name}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-card border border-border rounded-xl p-6 transition-all duration-200 hover:border-primary hover:-translate-y-[3px]">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">{sector.gbpCard.body} En {city.name}, esto te posiciona frente a la competencia local directa.</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-6 transition-all duration-200 hover:border-primary hover:-translate-y-[3px]">
              <h3 className="font-heading text-base mb-3">{sector.webCard.title}</h3>
              <p className="text-sm text-muted-foreground leading-relaxed">{sector.webCard.body} Adaptado al mercado de {city.name} y sus zonas.</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-6 transition-all duration-200 hover:border-primary hover:-translate-y-[3px]">
      ...
      {/* SECTION 6 — FAQ */}
      <FAQSection title={`Preguntas frecuentes para ${sectorLabel} en ${city.name}`} items={selectedFaqs} />
      ...
      <section className="py-12">
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div>
              <h3 className="font-heading text-lg mb-4">Más ciudades para {sectorLabel}</h3>
            ...
            <div>
              <h3 className="font-heading text-lg mb-4">Más sectores en {city.name}</h3>
              <div className="flex flex-wrap gap-2">
                {otherSectors.map((s) => (
                  <Link
                    key={s.slug}
